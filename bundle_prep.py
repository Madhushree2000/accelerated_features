...
def create_bundle_adjustment_data(calib_file, tum_file, image_dir, output_pkl):
    ...
    # =====================================================
    # MODIFICATION: Restrict to first 2 images and poses
    # =====================================================
    image_paths = image_paths[:2]  # Only keep first 2 images
    print(f"Using first 2 images: {[os.path.basename(p) for p in image_paths]}")
    
    # Ensure we have exactly 2 poses (pad with identity if needed)
    if poses_data:
        if len(poses_data) < 2:
            print(f"Padding TUM poses from {len(poses_data)} to 2 with identity...")
            # Create identity poses for missing entries
            for i in range(2 - len(poses_data)):
                poses_data.append({
                    'image_name': f"image_padded_{i:06d}.jpg",
                    'timestamp': 0.0,
                    'translation': np.zeros(3),
                    'quaternion': np.array([0, 0, 0, 1.0])  # Identity quaternion
                })
        else:
            poses_data = poses_data[:2]  # Only keep first 2 poses
    else:
        print("No TUM poses found, creating 2 identity poses")
        poses_data = [
            {
...
if __name__ == "__main__":
    ...
    TUM_FILE = "/Volumes/Lenovo PS6/Dataset/short_folder/short_data.txt"     # Path to your TUM file
    IMAGE_DIR = "/Volumes/Lenovo PS6/Dataset/short_folder"
    OUTPUT_PKL = "/Volumes/Lenovo PS6/Dataset/shortfolder_for_bundle_adjustment.pkl"
