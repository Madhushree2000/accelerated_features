...
class BundleAdjuster:
        ...
        with open(pose_file, 'r') as f:
            for line in f:
                ...
                quat = np.array([float(parts[5]), float(parts[6]), 
                               float(parts[7]), float(parts[8])])  # Note: w is last in your format
    ...
    def setup_bundle_adjustment_multiple_images(self, image_paths, poses_dict, all_keypoints, all_descriptors):
        ...
        # Track features across multiple images with improved matching
        point_id = 0
        
        # Match features between ALL pairs of images (not just consecutive)
        for i in range(len(image_paths)):
            for j in range(i + 1, len(image_paths)):
                print(f"Matching features between image {i} and {j}")
                
                # Match features
                matches = self.match_features(all_descriptors[i], all_descriptors[j], ratio_threshold=0.7)
                
                if len(matches) > 0:
                    # Get matched keypoints
                    kpts1 = all_keypoints[i][matches[:, 0]]
                    kpts2 = all_keypoints[j][matches[:, 1]]
                    
                    # Triangulate 3D points
                    points_3d = self.triangulate_points(
                        kpts1, kpts2,
                        self.poses[i][0], self.poses[i][1],
                        self.poses[j][0], self.poses[j][1]
                    )
                    ...
                    # Add valid points and observations
                    for k, (pt3d, kpt1, kpt2) in enumerate(zip(valid_points, valid_kpts1, valid_kpts2)):
                        ...
                        self.observations[j].append(kpt2)
                        self.point_indices[j].append(point_id)
    ...
    def optimize(self, max_iterations=200):
        ...
        n_cameras = len(self.poses)
        n_points = len(self.points_3d)
        total_observations = sum(len(obs) for obs in self.observations)
        total_residuals = total_observations * 2  # Each observation has x,y residuals
        total_parameters = n_cameras * 6 + n_points * 3
        
        print(f"Optimization parameters:")
        print(f"  - Cameras: {n_cameras} (parameters: {n_cameras * 6})")
        print(f"  - 3D points: {n_points} (parameters: {n_points * 3})")
        print(f"  - Total parameters: {total_parameters}")
        print(f"  - Total observations: {total_observations}")
        print(f"  - Total residuals: {total_residuals}")
        
        # Check if we have enough observations
        if total_residuals <= total_parameters:
            print(f"WARNING: Insufficient observations for optimization!")
            print(f"Need at least {total_parameters + 1} residuals, have {total_residuals}")
            print("Consider:")
            print("  1. Adding more images")
            print("  2. Improving feature matching (lower ratio threshold)")
            print("  3. Using only point parameters (fix camera poses)")
            
            # Option: Optimize only 3D points, fix camera poses
            print("Switching to points-only optimization...")
            return self.optimize_points_only(max_iterations)
        ...
        # Run optimization with robust method
        try:
            ...
            print(f"TRF method failed: {e}")
            print("Trying dogbox method...")
            result = least_squares(
                self.residual_function,
                initial_params,
                method='dogbox',
    ...
    def optimize_points_only(self, max_iterations=200):
        """Optimize only 3D points, keeping camera poses fixed."""
        print("Optimizing 3D points only (camera poses fixed)...")
        
        n_points = len(self.points_3d)
        total_observations = sum(len(obs) for obs in self.observations)
        total_residuals = total_observations * 2
        total_parameters = n_points * 3
        
        print(f"Points-only optimization:")
        print(f"  - 3D points: {n_points} (parameters: {total_parameters})")
        print(f"  - Total residuals: {total_residuals}")
        
        if total_residuals <= total_parameters:
            print("Still insufficient observations even for points-only optimization!")
            return None
        
        def points_only_residual(point_params):
            points_3d = point_params.reshape(n_points, 3)
            residuals = []
            
            for cam_idx in range(len(self.poses)):
                if len(self.observations[cam_idx]) == 0:
                    continue
                
                R_mat, tvec = self.poses[cam_idx]
                observed_points = self.observations[cam_idx]
                point_ids = self.point_indices[cam_idx]
        ...
        # Run optimization
        initial_params = self.points_3d.flatten()
        initial_residuals = points_only_residual(initial_params)
        initial_cost = 0.5 * np.sum(initial_residuals**2)
        print(f"Initial cost: {initial_cost:.6f}")
        
        result = least_squares(
            points_only_residual,
            initial_params,
            method='trf',
            max_nfev=max_iterations * len(initial_params),
            verbose=1
        ...
        # Update 3D points
        self.points_3d = result.x.reshape(n_points, 3)
        
        print(f"Points-only optimization completed!")
        print(f"Initial cost: {initial_cost:.6f}")
        print(f"Final cost: {result.cost:.6f}")
        print(f"Cost reduction: {((initial_cost - result.cost) / initial_cost * 100):.2f}%")
